...
const COLUMNS: ColumnConfig[] = [
  ...
  },
  {
    ...
    bgHeaderClass: 'bg-indigo-950/20',
    iconSvg: (
      ...
      {COLUMNS.map((col) => {
        ...
        return (
          <div
            ...
            className={`glass-card rounded-2xl flex flex-col min-h-[500px] border-t-4 ${col.borderAccentClass} transition-all duration-200 ${
              isHovered
                ? 'ring-2 ring-indigo-500/80 bg-slate-900/90 shadow-xl shadow-indigo-500/10'
                : 'bg-slate-950/40'
            }`}
          >
            {/* Column Header */}
            <div className={`p-4 rounded-t-xl flex items-center justify-between border-b border-slate-800 ${col.bgHeaderClass}`}>
            ...
            <div className="p-3 flex-1 flex flex-col space-y-3 overflow-y-auto max-h-[calc(100vh-280px)]">
              {columnTasks.length > 0 ? (
                ...
                <div className={`flex-1 min-h-[140px] rounded-xl border-2 border-dashed flex flex-col items-center justify-center p-6 text-center transition-colors ${
                  isHovered
                  ...
                  {!isHovered && onAddTaskInStatus && (
                    <button
                      onClick={() => onAddTaskInStatus(col.id)}
                      className="mt-2 text-[11px] text-indigo-400 hover:text-indigo-300 hover:underline font-semibold"
                    >
                      + Add Task
                    </button>
                  )}
