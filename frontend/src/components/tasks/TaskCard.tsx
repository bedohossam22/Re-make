  ...
  return (
    ...
    >
      <div>
        ...
        {/* Task Title */}
        <h3 className="text-base sm:text-lg font-bold text-slate-100 group-hover:text-indigo-300 transition-colors line-clamp-1 mb-2 leading-snug">
          {task.title}
        </h3>
      ...
      <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs">
        <div className="flex items-center space-x-1.5 min-w-0">
          ...
          <span className={`font-medium truncate ${overdue ? 'text-red-400 font-bold' : 'text-slate-300'}`}>
            {formatDate(task.dueDate)}
          </span>
          {overdue && (
            <span className="bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded text-[10px] uppercase font-bold border border-red-500/30 shrink-0">
              Overdue
            </span>
          )}
        </div>
